...
#add optimizer
value_model_optimizer = tf.keras.optimizers.Adam(learning_rate = 0.001)
...
for episode in range(episodes):
    ...
    for iter in range(sample_iter):
        ...
        #print("Episode: {}, ob: {}".format(episode, len(obs)))
        #print("obs: {}".format(len(obs)))
        #print("action: {}".format(len(actions)))
        #print("action prob: {}".format(len(action_probs)))
        G = 0
        for r in rs[::-1]:
            G = r + gamma * G
            Gs.insert(0, G)

        mean_total_reward[iter] = np.sum(rs)
        mean_entropy[iter] = entropy / len(actions)
        obs_all[iter] = obs
        actions_all[iter] = actions
        action_probs_all[iter] = action_probs
        Gs_all[iter] = Gs
    ...
    #print("All ob after {} sample iterations: {}".format(sample_iter, len(obs_all)))
    ...
    for batch_id in range(nbatches):
        ...
        action_probs_batch = action_probs_all[batch_id * batch_size: (batch_id + 1) * batch_size] ## access the certain data 2D
        ...
        policy_grads = policy_tape.gradient(policy_loss, policy_model.trainable_variables, unconnected_gradients = tf.UnconnectedGradients.ZERO)
        ...
        for _ in range(cg_iters):
            with tf.GradientTape() as fisher_tape:
                ...
                kl_grad = kl_tape.gradient(kl_div, policy_model.trainable_variables, unconnected_gradients = tf.UnconnectedGradients.ZERO)
                ...
                # print(kl_grad_prod)
            fisher_vector_prod = fisher_tape.gradient(kl_grad_prod, policy_model.trainable_variables, unconnected_gradients = tf.UnconnectedGradients.ZERO)
        ...
        with tf.GradientTape() as fisher_tape_shs:
            with tf.GradientTape() as kl_tape_shs:
                ...
                kl_div = tf.reduce_mean(
                    tf.reduce_sum(action_probs_old * tf.math.log(action_probs_old / action_probs), axis=1))
            kl_grad_shs = kl_tape_shs.gradient(kl_div, policy_model.trainable_variables, unconnected_gradients = tf.UnconnectedGradients.ZERO)
            ...
            # print(kl_grad_prod)
        fisher_vector_prod_shs = fisher_tape_shs.gradient(kl_grad_prod_shs, policy_model.trainable_variables, unconnected_gradients = tf.UnconnectedGradients.ZERO)
    ...
    ep_score.append(mean_total_reward)
