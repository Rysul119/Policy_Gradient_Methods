...
#declaring env
#env_names = ['ReacherBulletEnv-v0','InvertedPendulumPyBulletEnv-v0', 'AntPyBulletEnv-v0', 'HopperBulletEnv-v0', 'InvertedDoublePendulumPyBulletEnv-v0', 'HalfCheetahPyBulletEnv-v0', 'Walker2DBulletEnv-v0']
env_names = ['ReacherBulletEnv-v0']
for env_name in env_names:
    ...
    for upd in upds:
        for policy_lr in policy_learning_rates:
            for mult_lr in mult_lrs:
                df = pd.read_csv('logs/a2c/envs/'+env_name+'/lrs/plr='+str(policy_lr)+'mult='+str(mult_lr)+'upd='+str(upd)+'.csv')
                x = np.cumsum(df['l'])
